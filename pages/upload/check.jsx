...
const Check = ({}) => {
  ...
  const [imageCheck2, setImageCheck2] = useState(null);
  const validationSchema = yup.object({
    image1: yup.mixed().required("عکس روی چک را آپلود کنید"),
    image2: yup.mixed().required("عکس پشت چک را آپلود کنید"),
  ...
  const imageCheckHandler = async (e) => {
    if (e.target.files[0]) {
      ...
      formik.setFieldValue("document_id", data.split("_")[3]);
      formik.setFieldValue("uuid", data.split("_")[0]);
    }
  };
  const imageCheck2Handler = async (e) => {
    if (e.target.files[0]) {
      const file = e.target.files[0];
      const image = await resizeFile(file);
      setImageCheck2(URL.createObjectURL(file));
      formik.setFieldValue("image2", image);
    }
  };
  return (
    <main
        ...
        >
          ...
          <div className="h-full w-full gap-12  flex flex-col items-center">
            <div className="flex flex-row items-center w-[90%] min-w-fit h-fit rounded-cs border border-primary-200 p-2 text-sm justify-between">
              ...
              <ul className="flex gap-4 flex-col items-center text-gray-900 w-1/2">
                <li>{data.split("_")[5] ? data.split("_")[5] : ""} </li>
            ...
            <div className="w-full border-2 border-dashed border-primary-200 rounded-cs flex flex-col items-center gap-6 p-6">
              <p className="text-xs text-gray-900">
                لطفا عکس پشت و روی چک مورد نظر را بارگذاری کنید
              </p>
              <div className="w-full h-12">
                ...
                >
                  بارگذاری {imageCheck ? "مجدد" : ""} عکس روی چک{" "}
              ...
              <div className="w-full h-12">
                ...
                >
                  بارگذاری {imageCheck2 ? "مجدد" : ""} عکس پشت چک{" "}
                  <span className="mx-2 text-2xl">
                    <MdCloudUpload />
                  </span>
                </label>
              ...
              {imageCheck2 ? (
                <div className="h-24 w-56 relative">
                  <Image alt="check" src={imageCheck2} layout="fill" />
                </div>
              ) : null}
            </div>
          </div>

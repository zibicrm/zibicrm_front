  ...
  const iconBoxList = [
    {
      ...
      text: "آدرس مطب",
      link: `/admin/message/clinic/${id ? `?id=${id}&name=${name}` : ""}`,
      id: 0,
    },
    {
      icon: <MdOutlineAttachMoney />,
      text: " پیامک بیعانه",
      link: `/admin/message/deposit/${id ? `?id=${id}&name=${name}` : ""}`,
    ...
    },
    {
      icon: <MdOutlineChat />,
      text: "گفت و گو ها",
      link: `/admin/message/conversation/${id ? `?id=${id}&name=${name}` : ""}`,
      id: 2,
    },
  ];
  return (
    <Layout>
      <div className=" flex flex-col  h-full max-h-[calc(100vh-140px)]">
        ...
        <div className="flex flex-row h-full min-h-full max-h-full ">
          <div className="w-full h-full flex flex-row items-start gap-6 p-6 text-primary-900">

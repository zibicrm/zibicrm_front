        ...
        <div className="w-full flex flex-row items-center justify-between px-6 py-4">
          ...
          <div className="flex flex-row items-center gap-3">
            ...
            <div className="w-32 h-12">
              <PrimaryBtn
                text="افزودن پرونده"
                onClick={() => router.push("/operator/record/add")}
              >
          ...
          <table className="w-full  min-w-[600px] md:min-w-fit  max-w-full overflow-x-scroll  ">
            <thead className="bg-gray-50 h-[42px] min-w-[800px] md:min-w-fit  ">
              <tr className="text-right text-sm">
                <th className="w-20 text-gray-900 text-center border border-gray-200 relative">
                  ردیف
                </th>
                {head.map((item) => (
                  <th
                    key={item.id}
                    className=" text-right text-gray-900 border px-3 border-gray-200 relative"
                  >
                    {item.title}
                    {item.arrow ? (
              ...
              {filterRecord &&
                ...
                filterRecord.documents.map((item, index) => (
                  ...
                  <tr
                    ...
                    onClick={() =>
                      router.push(
                        `/operator/record/detail/?id=${item.id}`,
                        undefined,
                        { shallow: false }
                      )
                    }
                  >
                    ...
                    >
                      <IconBtn
                        icon={<MdRemoveRedEye />}
                        onClick={() =>
                          router.push(`/operator/record/detail/?id=${item.id}`)
                        }
                      />

...
const Users = ({}) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="w-[150px] h-[70px] bg-gray-900 text-gray-200 font-normal text-xs  bg-opacity-70 rounded shadow-none  outline-none ring-0 p-2 ">
          <div className="flex justify-between p-1 border-b-2 border-dotted">
            {/* <p className="mb-0">{payload[0].payload.day}</p> */}
            <span>{payload[0].payload.unVizited}</span>
            <p className="mb-0">{payload[1].name}</p>
          </div>
          <p className=" text-right flex flex-row justify-between  mt-2 p-1 h-1/2">
  ...
  const formatterData = (groups) => {
    ...
    for (var key in groups) {
      ...
      group.map((item) =>
        item.treatment ? (numOfVizit = numOfVizit + 1) : null
      );
      allVisit = group.length;
      chartD.push({
        day: key,
        visited: numOfVizit,
        unVizited: allVisit,
  ...
  return (
    <Layout>
      <div className="bg-white  p-6 flex flex-row items-center justify-between border-b border-gray-200">
